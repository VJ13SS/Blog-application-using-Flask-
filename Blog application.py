...
def home():
	if 'user' in session :
		...
		return render_template('Home_FP.html',current_user = user,display_posts = posts[: : -1])
...
def viewposts():
	...
	return render_template('Viewposts_FP.html',user_posts = posts[::-1])

#search user posts
@app.route('/search-user',methods = ['GET','POST'])
def search_user():
	if request.method == 'POST':
		searched_user = request.form['find_user']
...
def viewuser(id):
	...
	return render_template('UserPosts_FP.html',view_user = user,view_user_posts = user_posts[: : -1],total_posts = length)
